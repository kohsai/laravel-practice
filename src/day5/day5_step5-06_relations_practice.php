<?php
/**
 * 📘 Day5 教材（Step5-06：リレーションの実践 - 実際に定義して動作確認）【統合版】
 * 
 * この教材では「実際にモデルにリレーションを定義して動作確認する」方法を学びます
 * 
 * 【統合版について】
 * 基本教材 + 学習中に出た質問と回答を統合したバージョンです
 */
 ...
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * 
 * 「リレーションを定義すると、Laravelが自動的にSQLを組み立ててくれる」
 * とのことですが、SQLとは何ですか？
 * 自動的に組み立てられるメリットは？
 * Laravelを使わないとどうなりますか？
 */
 ...
 * 
 * 「背番号1番の本を持ってきて」
 * 「タイトルに『Laravel』が含まれる本を全部持ってきて」
 * 
 * このような指示をデータベースが理解できる形式で書いたものがSQL。
 * 
 * 【具体例】
 * 
 * Eloquent:  User::find(1)->expenses;
 * ↓ Laravelが自動生成
 * SQL:       SELECT * FROM expenses WHERE user_id = 1
 * 
 * 【自動生成のメリット】
 * 1. 書く量が減る（1行で済む）
 * 2. 安全（SQLインジェクション攻撃を防ぐ）
 * 3. タイプミスを防げる
 * 4. データベースの種類が変わっても同じコードで動く
 * 
 * 【Laravelを使わないと】
 * 
 * // Laravelあり（1行）
 ...
 * 
 * // Laravelなし（15行以上）
 * $pdo = new PDO('mysql:host=localhost;dbname=laravel_practice', 'root', 'password');
 ...
 * // さらに配列を1つずつ処理...
 * 
 * 全部自分で書く必要があり、時間がかかる＋エラーが出やすい。
 */

/**
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * 🙋 質問2: Laravelを使わない人はいるか？
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * 
 * Laravelを使わない人はいますか？
 * どんな場合に使わないのですか？
 ...
 * はい、たくさんいます。
 * 
 * 【ケース1: 他のPHPフレームワークを使う】
 * - Symfony（大企業向け）
 * - CodeIgniter（軽量）
 * - CakePHP（日本で人気）
 * 理由：会社がすでに採用している、プロジェクトに合っている
 * 
 * 【ケース2: PHP以外の言語を使う】
 * - Python（Django） - AI・機械学習と相性良い
 * - Ruby（Rails） - スタートアップで人気
 * - Node.js（Express） - フロント・バック統一
 * - Java（Spring） - 金融・大企業
 * 理由：言語の得意分野が違う
 * 
 * 【ケース3: WordPress】
 ...
 * 
 * 【重要】
 * 「Laravel以外はダメ」ではない。適材適所。
 * KOHさんはフリーランスなので、要件に応じて最適な技術を選べる。
 */
